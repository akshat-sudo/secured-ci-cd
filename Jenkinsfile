pipeline{
    ...
    stages{
        ...
        stage ('checkout SCM') {
            steps {
                git 'https://github.com/akshat-sudo/secured-ci-cd.git'
            }
        }
            ...
            steps{
                script{
                    withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                        sh "docker build -t sudoakshat/secured-ci-cd:${BUILD_TAG} ."
                        sh "docker push sudoakshat/secured-ci-cd:${BUILD_TAG}"
                   }
                }
            }
        }
        stage("Image Scanning using TRIVY"){
            ...
            steps{
                ...
                sh 'docker run -d --name qacontainer -p 8080:8080 sudoakshat/secured-ci-cd:latest'
     ...
     always {
        emailext attachLog: true,
            ...
            body: "Project: ${env.JOB_NAME}<br/>" +
                ...
                "URL: ${env.BUILD_URL}<br/>",
            to: 'sarangpawar451@gmail.com',  // Developer or manager Email
            attachmentsPattern: 'trivy.txt'
